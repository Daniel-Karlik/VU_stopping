...
class Experiment:
    ...
    def __init__(self, num_states: int, num_actions: int, num_steps: int, num_mc_runs: int,
                 horizon: int, ideal_s: np.ndarray, ideal_a: np.ndarray, w: float, mu: float,
                 len_sim: int, q: float, type_of_run: int) -> None:
        """
    ...
    def run(self) -> None:
        """
        Run experiment based on selected type_of_run and save results.
        """
        if self.type_of_run == 0:
            for i in range(self.num_mc_runs):
                t0 = time.time()
                mcs = MonteCarloSimulation(self.num_states, self.num_actions, self.num_steps, self.ideal_s,
                                           self.ideal_a, self.w, self.mu, self.known_model, self.horizon, self.q,
                                           self.len_sim)
                mcs.perform_single_run()
                t1 = time.time()
                self.load_results(mcs, i, self.type_of_run, t1 - t0)
                print(f"Iteration number: {i}")
        elif self.type_of_run == 1:
            for i in range(self.num_mc_runs):
                t0 = time.time()
                mcs = MonteCarloSimulation(self.num_states, self.num_actions, self.num_steps, self.ideal_s,
                                           self.ideal_a, self.w, self.mu, self.known_model, self.horizon, self.q,
                                           self.len_sim)
                mcs.perform_single_dynamic_run()
                t1 = time.time()
                self.load_results(mcs, i, 1, t1 - t0)
        else:
            for i in range(self.num_mc_runs):
                t0 = time.time()
                mcs = MonteCarloSimulation(self.num_states, self.num_actions, self.num_steps, self.ideal_s,
    ...
    def save_results(self):
        """
        Prints table of statistics of the runs and saves the results into .csv file
        :return:
        """
    ...
    def save_images(self):
        """
        Saves images of Histogram of stopping time, histogram of states, histogram of actions
        :return:
        """
...
class MonteCarloSimulation:
    ...
    def perform_single_run(self):
        ...
        for step in range(self.len_sim):
            ...
            # initialization before DP
            self.agent.t = self.horizon - 1
            self.agent.h[:, :] = 1
            # DP phase, where the optimal decision rule is obtained and use for generating next action
            for i in range(self.horizon):
                self.agent.evaluate_PE_short()
                self.agent.evaluate_r_o()
                self.agent.normalize_r_o()
                self.agent.t = self.agent.t - 1
            # We end DM cycle after horizon times iterations and use last agent.r_o
    ...
    def perform_single_dynamic_run(self):
        ...
        for step in range(self.len_sim):
            ...
            self.history[1] = current_state
            self.agent.estimate_model()
            # initialization of DP phase
            self.agent.t = self.horizon - 1
            self.agent.h[:, :] = 1
            self.agent.h_long[:, :, :] = 1
            self.agent.d_io_long.fill(0)
            # DP phase with possible stopping
